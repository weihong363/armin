...
更准确地说，Armin 是一个以任务为中心的异步 Agent 工作协作层。用户把任务交给本地或中国内地友好的 Agent 后，可以离开电脑；任务继续在远端推进，用户通过手机查看进展、追加指令、暂停/恢复、停止或确认完成。
...
长期方向上，Armin 希望让用户像“给任务打电话”一样和工作本身交互：用户不需要关心具体是哪一个 Agent、哪一个 session 或哪一个终端窗口，而是围绕某个任务持续补充上下文和做决策。当前 Phase 2 还不是完整的 Task Call 体验；当前实现仍基于任务详情页、语音/文本追加、SSH/tmux session 和任务审计链路。
